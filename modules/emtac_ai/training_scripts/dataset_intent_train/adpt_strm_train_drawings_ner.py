# modules/emtac_ai/training_scripts/dataset_intent_train/adpt_strm_train_drawings_ner.py
"""
Adaptive streaming NER training for 'drawings' with detailed logging.

What changed (logging-focused):
- Integrated your custom logger (modules.configuration.log_config):
  debug_id, info_id, warning_id, error_id, critical_id,
  with_request_id, set_request_id, get_request_id, log_timed_operation
- Request-scoped logs (each run tagged with a short request id).
- Timed sections around tokenizer/model init, dataset creation, train/eval/export.
- Label schemas are printed clearly before training and again on export.
- Optional: probe current model folder config.json before starting (--probe-model-folder-labels)

Behavior is otherwise identical to your original script.
"""
...
def _sorted_id2label_str(id2label):
    try:
        ...
        # If keys are already ints, or mixed
        items = sorted(
            id2label.items(),
...
def simple_compute_metrics_fn(p):
    # p can be an EvalPrediction or a (preds, labels) tuple depending on version
...
def main():
    ...
    set_request_id()
    rid = get_request_id()
    info_id("Starting adaptive streaming training (drawings NER)", rid)
    ...
    log_label_schema("Training schema", ID2LABEL, LABEL2ID, rid)
    ...
    # === Export BEST checkpoint to ORC_DRAWINGS_MODEL_DIR ===
    ...
    dst = Path(ORC_DRAWINGS_MODEL_DIR)
    ...
    # Optional quick demo (interactive)
